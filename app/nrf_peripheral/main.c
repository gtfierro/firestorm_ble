...
#define NUM_CHAR_HANDLES 20
uint16_t service_handles[10];
ble_gatts_char_handles_t char_handles[NUM_CHAR_HANDLES];

ble_gap_adv_params_t adv_params = {
  .type = BLE_GAP_ADV_TYPE_ADV_IND,
  .fp = BLE_GAP_ADV_FP_ANY,
  .interval = 0x140, //Every 200ms //BLE_GAP_ADV_INTERVAL_MIN,
...
void ble_handler(ble_evt_t *evt) {
  ...
  switch(evt->header.evt_id) {
    ...
    {
        int16_t spi_handle = -1;
        uint16_t wh = evt->evt.gatts_evt.params.write.handle;
        int i;
        for (i = 0; i < NUM_CHAR_HANDLES; i++)
        {
            if (char_handles[i].value_handle == evt->evt.gatts_evt.params.write.handle)
            {
                spi_handle = i;
                break;
            }
        ...
        if (spi_handle >= 0)
        {
            uint8_t tlen;
            uint8_t writebuf[27];
            tlen = evt->evt.gatts_evt.params.write.len;
            if (tlen > 24) tlen = 24;

            writebuf[0] = SPI_WRITE;
            writebuf[1] = (uint8_t) spi_handle;
            writebuf[2] = tlen;
            memcpy(writebuf+3, evt->evt.gatts_evt.params.write.data, tlen);
            spi_enqueue_cmd(writebuf, 27);
            break;
        } else {
            uint8_t debug_cmd[20];
            debug_cmd[0] = SPI_DEBUG;
            snprintf((char *)debug_cmd + 1, 20, "WCN%d.", wh);
            spi_enqueue_cmd(debug_cmd, 20);
            break;
        }
    }
  }
...
void add_characteristic_cmd(uint8_t* req) {
  ...
  meta.char_props.write         = 1;
  meta.char_props.notify        = 1;
  meta.p_char_user_desc         = (uint8_t *)"Random";
  meta.char_user_desc_max_size  = strlen((char*) "Random");
  meta.char_user_desc_size      = strlen((char*) "Random");

  APP_ERROR_CHECK(sd_ble_gatts_characteristic_add(BLE_GATT_HANDLE_INVALID/*service_handle*/, &meta, &char_attr,
         &char_handles[spi_char_handle]));
  /*{
      uint8_t debug_cmd[10];

      debug_cmd[0] = SPI_DEBUG;
      sprintf(debug_cmd + 1, "AC%d.%d.", char_handles[spi_char_handle].value_handle,spi_char_handle);
      spi_enqueue_cmd(debug_cmd, 10);
  }*/
}

void dmsg(char* msg)
{
    uint8_t debug_cmd[12];
    snprintf((char*)debug_cmd+1,10,"%s",msg);
    debug_cmd[0] = SPI_DEBUG;
    spi_enqueue_cmd(debug_cmd, 12);
}
...
void getvalue_cmd(uint8_t *rx_buf) {
    uint8_t spi_handle = rx_buf[1];
    uint16_t handle = char_handles[spi_handle].value_handle;
    uint16_t len = 24;
    uint8_t buf [27];
    ble_gatts_value_t attribute = {
        .len = len,
        .offset = 0, // attribute value offset?
        .p_value = &buf[3]
    };
    APP_ERROR_CHECK(sd_ble_gatts_value_get(handle, spi_handle, &attribute));
    buf[0] = SPI_GETVALUE;
    buf[1] = spi_handle;
    buf[2] = (uint8_t) len;
    spi_enqueue_cmd(buf, 27);
}
    ...
    case SPI_SLAVE_XFER_DONE:
      ...
      switch (spi_rx_buf[0]) {
        ...
        case SPI_START_ADVERTISING: // start advertising;
          nrf_gpio_pin_set(LED);
          if (spi_rx_buf[1] != 0) {
            APP_ERROR_CHECK(sd_ble_gap_adv_data_set(&spi_rx_buf[2], spi_rx_buf[1], NULL, 0));
          }
          //
          APP_ERROR_CHECK(sd_ble_gap_adv_start(&adv_params));
          break;
